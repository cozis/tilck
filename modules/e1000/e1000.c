...
{
    if (is_mmio) {
        return *(volatile u32*) (io_addr + off);
    } else {
...
static int setup_tx_ring(void)
{
    ...
    size_t ring_pages = CEIL(ring_bytes, PAGE_SIZE);
    size_t data_pages = CEIL(data_bytes, PAGE_SIZE);
...
determine_bar0_addr_space(struct pci_device_loc loc)
{
    ...
    u32 num_pages = CEIL(num_bytes, PAGE_SIZE);
